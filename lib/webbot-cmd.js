...
Q().
  ...
  then(function (resourceDoc) {
    ...
    if (scenarioConfig.jsonScenario) {
      ...
      webBot.buildScenarioFromJson(options, deferred.makeNodeResolver());
      ...
      webBot.buildScenarioFromDlJson(options, deferred.makeNodeResolver());
      ...
      if (program.tojson) {
        ...
        webBot.buildScenarioFromGoogleDoc(options, deferred.makeNodeResolver());
      }
    } else {
  ...
  then(function (resourceDoc) {
    if (resourceDoc) {
      logger.trace('scenario resources :', JSON.stringify(resourceDoc));
      process.exit(1);
    }
