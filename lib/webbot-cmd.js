...
function googleAccount(account) {
  var accountSpecs = account.split(':');
  return {
...
program.
  ...
  option('-o, --tocsv <jsonfile>', 'return a csv content based on the specified json scenario').
...
Q().
  ...
  niceThen(function (next, resolve) {
    ...
    if (scenarioConfig.resScenario) {
      ...
      util.downloadJsonUri(scenarioConfig.resScenarioUri, next);
  ...
  niceThen(function (resourceDoc, next) {
    var options;
    if (program.tocsv) {
      util.jsonToCsv(program.tocsv, function (err, csvDoc) {
        if (err) {
    ...
    } else if (scenarioConfig.jsonScenario) {
      options = {
