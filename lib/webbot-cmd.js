...
argv = yargs
  .usage(util.format('Usage: %s --env [env] --scenario [scenario] --loglevel [trace|info|warn|error] --capabilities [desired capabilities] --commands [extra client commands]', pkg.name))
  .options({
    'dir': {
  ...
  .example(util.format('%s -e live -s 1-simple -c \'{"desiredCapabilities":{"browserName": "firefox"}}\' -l trace', pkg.name), 'Execute my-super-test scenario on production with firefox and the log level trace')
  .argv;
