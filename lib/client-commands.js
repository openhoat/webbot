...
clientCommands.checkLocalFile = function (file, content, timeout, callback) {
  ...
  setTimeout(function () {
    ...
    clearInterval(interval);
  }, config.file && config.file.download && config.file.download.timeout || 30000);
  return browser;
};
...
clientCommands.waitAndCheckText = function (selector, timeout, expectedText, callback) { // Attend qu'un selector aie la valeur attendue
  ...
  browser
    ...
    .getText(selector, webbot.helpers.expectValueChecker(expectedText))
    .call(callback);
  return browser;
};

clientCommands.waitForSomething = function (selector, someFunc, someArgs, timeout) { // Waits for something defined by function
  var callback
    , browser = this
    , response = {};
  ...
  async
    .waterfall([
      function (cb) {
        browser.timeoutsAsyncScript(timeout, cb);
      },
      function (res, cb) {
        response.timeoutsAsyncScript = res;
        browser.selectorExecuteAsync(selector, someFunc, someArgs, cb);
      },
