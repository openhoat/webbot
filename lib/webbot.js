  ...
  return new Promise(
    function (resolve/*, reject*/) {
      function registerClientCommand(commands) {
        ...
        log.trace('registered client commands :', commands);
      }
  ...
  return new Promise(
    function (resolve/*, reject*/) {
      http.get(opt, function () {
        log.trace('selenium server response detected');
        resolve(true);
      }).on('error', function (/*err*/) {
        log.trace('no selenium server responding');
        resolve(false);
      });
    })
    .nodeify(cb);
...
webbot.init = function (opt, cb) {
  ...
  return webbot
    ...
    .then(function (running) {
      ...
      log.trace('selenium server is already running');
    })
    .then(function () {
...
webbot.initBrowser = function (opt, cb) {
  ...
  opt = args.opt;
  cb = args.cb;
  opt.verbose && log.trace('initialize browser with options :', opt);
  return new Promise(
    function (resolve/*, reject*/) {
  ...
  return new Promise(
    function (resolve/*, reject*/) {
      ...
      try {
        ...
        log.trace('scenario default config loaded');
      } catch (err) {
      ...
      if (state.scenario.env) {
        try {
          ...
          log.trace('scenario config loaded');
        } catch (err) {
          if (err.code === 'MODULE_NOT_FOUND') {
            log.warn(err);
          } else {
      ...
      log.trace('scenario config : ', state.scenario);
      resolve();
    })
    .nodeify(cb);
};
...
webbot.playScenario = function (opt, cb) {
  ...
  return webbot
    ...
    .then(function () {
      var scenario = webbot.getScenario();
      if (scenario.timeout) {
    ...
    .then(function () {
      ...
      return new Promise(
        function (resolve/*, reject*/) {
          mocha.addFile(path.join(process.cwd(), 'scenarii', '1-simple'));
          mocha.run(function (failures) {
            log.trace('mocha failures :', failures);
...
webbot.startSelenium = function (opt, cb) {
  ...
  return new Promise(
    function (resolve/*, reject*/) {
      log.trace('starting selenium server');
      ...
      ['stderr', 'stdout'].forEach(function (output) {
        state.server[output].on('data', function (data) {
          ...
          opt.verbose && log.trace('selenium - %s - %s', output, line);
    ...
    .then(function () {
      log.trace('selenium server started');
    })
    .nodeify(cb);
};
...
webbot.stopSelenium = function (opt, cb) {
  ...
  return new Promise(
    ...
    .then(function (result) {
      if (result) {
        log.trace('selenium server stopped');
      } else {
        log.trace('selenium server not running');
      }
      return result;
    })
    .nodeify(cb);
