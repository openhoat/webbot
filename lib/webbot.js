    ...
    function (resolve/*, reject*/) {
      function registerClientCommand(commands) {
        ...
        Object.keys(commands).forEach(function (name) {
          ...
          log.debug('registered client command :', name);
        });
      }
...
webbot.loadScenarioConfig = function (opt, cb) {
  ...
  return new Promise(
    function (resolve/*, reject*/) {
      state.scenario = {
        env: opt.scenario.env, name: opt.scenario.name
      };
      if (!state.scenario.name) {
        throw new Error('no scenario name specified');
      }
      try {
        scenarioConfig.default = require(path.join(opt.baseDir || config.baseDir, 'config', 'scenarii', 'default', state.scenario.name)); // load the common configuration for all environments
      ...
      if (state.scenario.env) {
        try {
          scenarioConfig.scenario = require(path.join(opt.baseDir || config.baseDir, 'config', 'scenarii', state.scenario.env, state.scenario.name));
...
webbot.playScenario = function (opt, cb) {
  ...
  return webbot
    ...
    .then(function () {
      return new Promise(
        function (resolve/*, reject*/) {
          var mocha, scenarioFile;
          mocha = new Mocha(opt.mocha);
          scenarioFile = path.join(opt.baseDir || config.baseDir, 'scenarii', scenario.name);
          log.trace('adding scenario :', scenarioFile);
          mocha.addFile(scenarioFile);
