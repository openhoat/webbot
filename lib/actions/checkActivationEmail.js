...
function fetchEmail(step, options, searchCriteria, fetchCriteriaFunc, logger, callback) {
  ...
  Q().
    ...
    niceThen(function (box, next, resolve) {
      mailBox = box;
      if (searchCriteria) {
        imap.search(searchCriteria, next);
      } else {
        resolve();
      }
    }).
...
that = {
  build: function (webBot, step) {
    ...
    return function (done) {
      ...
      login = util.getItemParam(step, 10);
      pwd = util.getItemParam(step, 11);
      ...
      function checkEmail(err, emails) {
        var i, email, body, matches, activationLink;
        if (err) {
          return done(err);
        }
        if (emails && emails.length) {
          ...
          email = emails[0];
          logger.trace('email body :', email.body);
          if (email.body) {
            body = email.body.split('=\r\n').join('');//.split('\r\n').join('');
            logger.trace('checking body :', body);
